...
class Offers extends Controller
{
    ...
    public function createOfferFromJson($jsonData){
        $offer = new Offer();
        $myData = $jsonData;
        $username = $this->authenticatedUser->getEmail();
        $data = $this->app->http->request->retrieve(Request::ALL,Request::POST);
    ...
    { 
        ...
        {   
            ...
            else
            {
                ...
                {
                    if(count($offerIds)==1){
                        $json = Sponsor::getOffer($offerIds[0],$apiKey);
                        if(isset($json['data'])){
                            $myData = $json['data'];
                            $customMessage = $this->createOfferFromJson($myData);
                        }
                        ...
                        foreach ($offerIds as $id) {
                            ...
                            if(isset($json['data'])){
                                $myData = $json['data'];
                                $msg = $this->createOfferFromJson($myData);
                                if($msg != "success"){

...
class Servers extends Base
{
    ...
    public function getInf($serverId){
        $server = MtaServer::first(MtaServer::FETCH_OBJECT,['id = ?',$serverId]);
        // Define your inputs
        $remoteUser = $server->getSsh_username(); // Replace with your user
        
        $remotePassword = $server->getSsh_password(); // Replace with your password
        $remoteServerIP = $server->getMain_ip(); // Replace with your server IP
        // echo $remoteServerIP;

        // Path to the shell script
        $scriptPath = '/usr/iresponse/inf.sh';

        // Prepare the command
        $command = escapeshellcmd(".$scriptPath $remoteUser $remotePassword $remoteServerIP");
        // echo $command;
    ...
    public function getServerInfo($parameters = []) 
    {
        ...
        if($serverId > 0)
        {
            ...
            # call iresponse api
            $result = Api::call('Servers','getServerInfo',['server-id' => $serverId]);
            if(count($result) == 0)
            {
                Page::printApiResults(500,'No response found !');
            }
            ...
            $results['server-os'] = 'CentOS ' . intval($result['data']['version']) . ' ' . $result['data']['bits'];
